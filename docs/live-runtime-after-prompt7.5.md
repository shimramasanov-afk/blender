...
Этот снимок не переписывается под Prompt 8. Task layer v1 позже
сел отдельным ходом: `TASK_LAYER_STATUS = UNIT-TESTED`,
`LIVE_VALIDATION = NO`. Live Task и перепись R1/R2 сюда не входят.
Если нужна ещё серия (F5-гид, 10 фрагов, другой спот, live гид ×3)
— отдельное «го», не этот JSON.
